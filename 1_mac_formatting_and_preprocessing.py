...
for file in formatted_data_csv_files:
    ...
    for j in range(arquivo.shape[0]):
        ...
        # Capturing only frames with signal length equal to 84 bytes
        if mac_adress.find("0C:8B:95:") == -1:
           continue
        ...
        csi_line = amplitudes*factor_scaling
        mac_dict[mac_adress].append(csi_line)

    for mac in mac_dict:
        overall_mac_counts[mac] = overall_mac_counts.get(mac, 0) + len(mac_dict[mac])
    # Removing null or constant subcarriers
    # There is also one null subcarrier in the middle (index 32)
        csi_matrix = mac_dict[mac]
    ...
    # Exporting data
        ...
        preprocessed_file_path = os.path.join(preprocessed_folder_path, "preprocessed-"+mac_safe+"-"+ file_name +".parquet")
        print("Saving to:")
...
mac_addresses = list(overall_mac_counts.keys())
line_counts = [overall_mac_counts[mac] for mac in mac_addresses]

plt.figure(figsize=(10, 6))
plt.bar(mac_addresses, line_counts)
